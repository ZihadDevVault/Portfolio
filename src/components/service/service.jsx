...
const Service=({isDarkMode})=>{
    ...
    <div id="Sevices" className="w-full px-[12%] py-10 scroll-mt-20">
        ...
        {serviceData.map(({icon,title,description,link},index)=>{
            return(
                <div key={index} className="border border-gray-400 rounded-lg px-8  py-12 hover:shadow-black cursor-pointer hover:bg-lighHover  hover:-translate-y-1 duration-500 dark:hover:bg-darkHover dark:hover:shadow-white">
                    <img src={icon} alt="icon" className="w-10" />
                    <h4 className="text-lg my-4 text-gray-700 dark:text-white">{title}</h4>
